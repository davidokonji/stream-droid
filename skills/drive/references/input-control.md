...
```bash
bun scripts/drive.mjs tap 0.5 0.5             # tap a normalized point
bun scripts/drive.mjs longpress 0.5 0.5 800   # press and hold (ms optional, default 500)
bun scripts/drive.mjs swipe 0.5 0.8 0.5 0.2   # swipe — this one scrolls up
bun scripts/drive.mjs scroll 0.5 0.5 0.5      # scroll at a point by dy (dx optional)
bun scripts/drive.mjs text "hello world"      # type into the focused field
bun scripts/drive.mjs key Home                # press a named key (below)
bun scripts/drive.mjs tap:text "Network & internet"   # tap by element text/desc
bun scripts/drive.mjs tap:id search                    # tap by resource-id
```

From a clone the path is `bun skills/drive/scripts/drive.mjs …`; installed as a
plugin, `bun "$CLAUDE_PLUGIN_ROOT/skills/drive/scripts/drive.mjs" …`.
